...
// QueryTiles chains the current query on the "tiles" edge.
func (_q *UserQuery) QueryTiles() *TileQuery {
	query := (&TileClient{config: _q.config}).Query()
...
// WithTiles tells the query-builder to eager-load the nodes that are connected to
// the "tiles" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithTiles(opts ...func(*TileQuery)) *UserQuery {
	query := (&TileClient{config: _q.config}).Query()
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withTiles; query != nil {
		if err := _q.loadTiles(ctx, query, nodes,
			func(n *User) { n.Edges.Tiles = []*Tile{} },
			func(n *User, e *Tile) { n.Edges.Tiles = append(n.Edges.Tiles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadTiles(ctx context.Context, query *TileQuery, nodes []*User, init func(*User), assign func(*User, *Tile)) error {
	...
	query.withFKs = true
	query.Where(predicate.Tile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TilesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_tiles
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_tiles" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_tiles" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
